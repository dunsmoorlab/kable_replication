...
for exp in [2,4]:
    ...
    #only concerned about high confidence here
    #removing all low confidence old responses before analysis
    ...
    avg = df.groupby(['phase','condition','subject'])[['High']].mean() 
    ...
    normality = pg.normality(cr).loc[['High']].reset_index().rename(columns={'index':'confidence'})
    ...
    # plt.tight_layout()
    # plt.savefig(f'figures/{experiment}_author_response_results.png')
